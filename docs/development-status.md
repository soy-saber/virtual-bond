...
- 桌宠显示比例上限已经提高到 `180%`，方便近距离检查脸部与服装细节。
...
1. 实机验收皮肤设置页的预览、重新扫描、错误列表和跨模式同步。
2. 建立通用动作状态机、自动水平翻转和屏幕安全活动范围。
3. 增加图片生成设置、参考图选择、任务进度与历史记录；实际生图仍需逐次批准。
4. 通过 OpenCode CLI 接入隔离的 Agent 能力。
5. 开发长期记忆与关系成长。
