...
## 46. 双击桌宠时闪现旧立绘，单击与拎起同时触发

### 现象

- 双击桌宠进入陪伴空间时，扩大的窗口会短暂显示桌宠立绘。
- 按下人物准备拖动时，单击回应和拎起动作会同时播放。

### 原因与解决方案

- 原实现先调用主进程切换房间窗口尺寸，再更新 Vue 视图，因此旧桌宠组件会在扩窗后的一个渲染间隙内仍然可见。现在先设置房间状态并等待 Vue 卸载桌宠，再请求 Electron 切换窗口模式。
- 原实现把 `pointerdown` 直接视为拖拽开始，同时 `mousedown` 又立即触发单击回应。现在按下只记录候选点，移动超过 `6px` 后才开始拖拽；未超过阈值的释放才参与单击和双击判定。
- 单击回应延迟到双击判定窗口后执行，第二次单击会取消待执行回应并直接打开陪伴空间；`pointercancel` 只结束候选或拖拽，不会触发单击。

## 47. 行走、拎起和单击 Sprite Sheet 有帧但动作不成立

### 现象

- Q 版行走图播放时像侧面贴图平移，没有完整迈步感。
- 桌宠拎起时双脚向前，像在走路或坐姿，不符合悬挂受力。
- 单击动作播放过快，预备、回应和收势无法辨认。

### 原因与解决方案

- 行走 Sheet 缺少接触、下压、经过、抬升以及左右腿交换的完整循环；运行时添加上下起伏、旋转或阴影缩放只能制造抖动，不能补出缺失关键帧，因此已撤回这类补偿。
- 多层空间现在明确区分表现资源：移动时使用侧面行走 Sheet，到达或对话暂停时使用从 Q 版设定板提取的正面透明帧。
- 单击 Sheet 暂时由 `8 fps` 放慢到 `5 fps`，但单击和拎起仍列为待重绘正式素材。后续根据用户 GIF 把拎起规格进一步收敛为后颈/衣领受力、主动屈膝抱胸和整体惯性摆动；单击应包含预备、回应、停留和收势。

## 48. 生图动作 Sheet 的后处理与 Python 3.8 兼容

### 现象

- WisArt 返回的绿色背景 Sheet 需要去色键、统一脚底和水平中心后才能进入运行资源。
- 生成图偶尔在人物外产生孤立色点，直接归一化会把色点也纳入包围盒。
- 本机 Python 3.8 执行旧版归一化脚本时不支持 `zip(..., strict=True)`。

### 原因与解决方案

- v3 资源统一通过 `remove_chroma_key.py`、`remove-small-sprite-components.py` 和 `normalize-sprite-sheet.py` 处理；旧运行资源不覆盖，最终文件使用版本化名称。
- `normalize-sprite-sheet.py` 改为显式检查帧和包围盒数量后使用兼容 Python 3.8 的 `zip`，避免依赖 3.10 的 `strict` 参数。
- 生成调用使用当前本机 WisArt Base URL 和隔离 Key；Key 只注入当前命令环境，不写入仓库、日志或 Prompt。

## 49. v3 动作可以播放，但步频、姿势和切换流畅度仍未通过验收

### 现象

- `walk-right-v3` 已有前伸、经过、后蹬和换腿相位，但实机播放步频偏快。
- `dragging-v3` 虽然不再把脚伸成走路姿势，整体仍接近站姿悬空，缺少可爱的蜷缩悬挂感。
- `interaction-v3` 某些帧出现额外手掌，缩小后动作语义仍不稳定。
- 单击、说话、拎起和待机之间切换时出现明显停顿，增加图片帧数后可能进一步放大。

### 原因与后续方案

- 行走问题先从降低 `MultiFloorScene` 中的播放速度处理，再判断是否仍需重绘相位。
- 用户提供的直接操作 GIF 表明，目标拎起姿势应以颈后为固定提点，角色主动收腹、卷起骨盆、屈膝并腿、双臂靠近胸口，身体作为一个整体随拖拽速度滞后摆动；这不是四肢自然垂下的写实悬挂，也不采用完整布娃娃模拟。
- 下一版拎起拆成 `pickup / held-idle / release`，分别使用 12、24–36、10–12 帧并以 `24 fps` 播放；主体位置和倾角由 `60 fps` 弹簧阻尼计算。
- 该轮定位时，`PetSpritePlayer` 每次动作切换都会重新调用 `skins:load-animation`、创建 Blob URL、解码整张 PNG 并销毁旧纹理，因此先把按皮肤和动作缓存列为工程修复项。
- 后续提交已实现纹理缓存、动作锚点和 60 fps 运行时弹簧物理，并为三段动作提供旧皮肤兼容接口；正式高帧率分段资源及完整 `pickup → held-idle → release` 切换仍未完成。

## 50. 12 帧 pickup 草稿网格正确，但角色越界和受力点漂移

### 现象

- 首张 `4 × 3`、12 帧草稿能表现站立到屈膝抱胸，但部分脚部和头发接触或越过单格边界。
- 后颈/衣领受力点没有固定在同一局部坐标，动作看起来是人物在格子内移动，而不是身体绕固定提点转动。
- 最后几帧接近竖直悬空坐姿，侧向蜷曲角度不足。

### 原因与解决方案

- “保持同一像素坐标”和“保留足够留白”对图片模型仍不够量化；模型会为不同姿态分别重构图，使站姿脚底、蜷曲头发和受力点自行漂移。
- 失败稿只保存到 `resources/concepts/desktop-pet-pickup-sheet-v4-draft-key.png`，不接入 manifest，也不通过后处理掩盖已经丢失的边缘内容。
- 重试 Prompt 明确整图为 `1536 × 1152`、每格 `384 × 384`、后颈坐标 `(192, 88) ± 6px`、四边至少 24px 安全边距，并要求最终躯干形成 35–55 度侧向 C 形。
- 再次生图前仍需记录模型、尺寸、质量、参考图数量和用途并取得明确批准。
