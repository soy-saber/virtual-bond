...
- 单击 Sheet 暂时由 `8 fps` 放慢到 `5 fps`，但单击和拎起仍列为待重绘正式素材。拎起应使用头顶或衣领受力、躯干垂直悬挂、腿部下垂以及头发/外套滞后摆动；单击应包含预备、回应、停留和收势。
...
- `PetSpritePlayer` 当前每次动作切换都会重新调用 `skins:load-animation`、创建 Blob URL、解码整张 PNG 并销毁旧纹理。应先增加按皮肤和动作缓存，皮肤切换或组件卸载时再集中释放。
- 当前提交只记录并冻结 v3 候选、后处理管线和上述调研结论；高帧率分段资源、运行时物理和纹理缓存尚未实现。
