...
- 陪伴空间已加入本地 CSS 构建的“雨夜书房”三层景深与材质光照：玻璃雨窗和城市远景、书架与阅读椅、木地板和地毯、小桌热饮与植物前景，并支持减少动态效果偏好
...
- 将陪伴空间从全身立绘展示改为 Q 版房间形态，使用 PixiJS 场景图、脚底深度排序、可行走区域和家具互动锚点
...
- 桌宠模式继续使用高清全身形态，陪伴空间使用同一角色的 Q 版房间形态
- 先支持用户主动选择学习、用餐、休息和自由陪伴模式，再接入可编辑日程
- 将用户情境统一转换为伙伴意图，再选择房间、家具锚点和动作
- 学习时可在书桌执行阅读或查资料表现，用餐时移动到餐桌，休息时选择座椅或窗边
- 为情境来源、优先级、中断和回退提供可解释状态，不直接监控用户前台应用
